...
@section('title', 'Guides — Learn Hub — GOFX')
        ...
        @foreach($guides as $g)
          <article class="bg-slate-800/30 rounded-2xl p-6">
            <h3 class="text-white text-xl font-semibold mb-2">
              <a href="{{ route('guides.show', $g->slug) }}">{{ $g->title }}</a>
            </h3>

            <p class="text-slate-300 mb-4">{{ $g->excerpt }}</p>

            <div class="flex items-center justify-between">
              <div class="text-slate-400 text-sm">By {{ $g->author ?? 'GOFX' }}</div>
              <a href="{{ route('guides.show', $g->slug) }}" class="text-sm font-semibold text-slate-200">Read →</a>
            </div>
          </article>
        @endforeach
      </div>
